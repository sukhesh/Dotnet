...
public partial class moderator_mMails : System.Web.UI.Page
{
    ...
    {
        st = @"SELECT name, email_id, type, description, convert(varchar(10),e_date,101) as e_date, e_t_id FROM 
                email_tech_support WHERE (e_t_id NOT IN (SELECT e_t_id FROM responce_email_tech_support AS 
                responce_email_tech_support_1))";
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            st = " insert into responce_email_tech_support(e_t_id,reply,e_date,ent_by,status) values(" + GridView1.DataKeys[row.RowIndex].Value.ToString() + ",'" + tb_desc.Text + "',convert(datetime,'" + DateTime.Now.ToString("MM/dd/yyyy") + "',101),'" + Session["f_name"].ToString() + "',0) ";
            x = db.ExeQuery(st);
            if (x == 1)
            {
